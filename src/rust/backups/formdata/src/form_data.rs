...
impl FormData {
    ...
    pub fn to_multipart(&self) -> Result<Vec<Node>, Error> {
        ...
        for &(ref name, ref value) in &self.fields {
            ...
            //h.set(ContentType(Mime(TopLevel::Application, SubLevel::WwwFormUrlEncoded, vec![])));
        ...
        for &(ref name, ref file) in &self.files {
            ...
            h.set(ContentDisposition {
                disposition: DispositionType::Ext("form-data".to_owned()),
                parameters: vec![DispositionParam::Ext("name".to_owned(), name.clone()),
                                DispositionParam::Ext("filename".to_owned(), "ccc.png".to_owned())],
            });
            h.set(ContentType(Mime(TopLevel::Image, SubLevel::Png, vec![])));
            nodes.push(
                Node::File( h, file.to_vec() )
            );
        }
        Ok(nodes)
    }
