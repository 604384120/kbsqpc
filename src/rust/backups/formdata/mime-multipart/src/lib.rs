    ...
    -> Result<usize, Error>
{
    ...
    for node in nodes {
        ...
        match node {
            ...
            &Node::File(ref headers, ref files) => {
                // write the part's headers
                ...
                //let fileStr = String::from_utf8(files.to_vec()).unwrap();

                //count += try!(stream.write_all_count(fileStr.as_bytes()));
                //count += try!(stream.write_all_count(files));
                let mut file: &[u8] = &files;
    ...
    -> Result<(), Error>
{
    for node in nodes {
        ...
        match node {
            ...
            &Node::File(ref filepart, ref files) => {
                // write the part's headers
